...
app.post('/api/create-order', async (req, res) => {
  try {
    ...
    const razorpayMessage = e?.error?.description || e?.message || 'Unknown error while creating the payment order.';
    console.log('create-order failed:', JSON.stringify(e));
    res.status(500).json({ error: razorpayMessage });
  }
});
